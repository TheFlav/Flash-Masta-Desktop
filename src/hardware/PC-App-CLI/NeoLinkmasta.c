...
*/
    ...
    {
        printf("\rSuccess!  Verified %d bytes in %ld seconds.\n", addr, time(NULL)-startTime);
        fflush(stdout);
    }
...
void backupCartSaveGame()
{
    ...
    sprintf(ngfFilename, "%s-%ld.ngf", cartRomName, time(NULL));
	...
	{
		fprintf(stderr, "backupCartSaveGame: wrote %zu bytes, but exptected %lu bytes\n", bytes, sizeof(struct NGFheaderStruct));
		fclose(ngfFile);
		return;
	}
	...
	for(i=0;i<35;i++)
	{
		if(prot[0][i]==0)//writable block
		{
			...
			{
				fprintf(stderr, "save-game: wrote %zu bytes, but exptected %lu bytes\n", bytes, sizeof(struct blockStruct));
				fclose(ngfFile);
				return;
			}
			...
			{
				fprintf(stderr, "save-game: wrote %zu bytes, but exptected %d bytes\n", bytes, blockSize(i));
				fclose(ngfFile);
				return;
			}
		}
...
void restoreSaveGameToCart(char *ngfFilename)
{
  ...
  int i;
    ...
    if(!blockMem)
    {
		printf("restoreSaveGameToCart: can't malloc %lu bytes\n", (NGFheader.fileLen - sizeof(struct NGFheaderStruct)));
        return;
    }
	...
	{
		printf("restoreSaveGameToCart: read 0x%zX bytes, but exptected 0x%lX bytes\n", bytes, (NGFheader.fileLen - sizeof(struct NGFheaderStruct)));
		free(blockMem);
		return;
	}
	...
	printf("Wrote 0x%X bytes in %ld seconds.", totalWritten, time(NULL)-startTime);
        fflush(stdout);
...
int Dump32MbitCart()
{
    ...
    printf("\rBacked up %d bytes in %ld seconds\n", addr, time(NULL)-sTime);
    fflush(stdout);
...
void queryMicroSD(void)
{
    ...
    SD_SPI_transmit((uint8_t*) "\x40\x00\x00\x00\x00\x95", recvBuf, 6);
    ...
    SD_SPI_transmit((uint8_t*) "\x41\x00\x00\x00\x00\xFF", recvBuf, 6);
    ...
    SD_SPI_transmit((uint8_t*) "\x77\x00\x00\x00\x00\x95\xFF\xFF", recvBuf, 8);

    SD_SPI_transmit((uint8_t*) "\x69\x00\x00\x00\x00\x95\xFF\xFF", recvBuf, 8);
