...
{
  if (!FlashMastaApp::get_instance()->get_device_manager()->claim_device(m_id))
  {
    // Do nothing
    return;
  }
  ...
  linkmasta_device* linkmasta = FlashMastaApp::get_instance()->get_device_manager()->get_linkmasta_device(m_id);
  ...
  catch (std::runtime_error& e)
  {
    ...
    FlashMastaApp::get_instance()->get_device_manager()->release_device(m_id);
    return;
  }
